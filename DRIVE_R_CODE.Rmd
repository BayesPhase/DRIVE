---
title: "DRIVE Study Analysis Code"
author: "Jianzheng Wu"
date: "2023-11-21"
output:
...
Main_Data <- read_csv("S:/Biostats/BIO-STAT/BISR/FACTS Working Group/FACTS code/Case Study 22 DRIVE/6.Main Analysis/Analysis_JZ/First_Interim/Raw_data/DRIVETrial_Initial_Phase.csv")
...
#folder="S:/Biostats/BIO-STAT/BISR/FACTS Working Group/FACTS code/Case Study 22 DRIVE/6.Main Analysis/Analysis_JZ/First_Interim/R_analysis"
#write.csv(Randomization_list,paste(folder,"Randomization_list1.csv",sep="/"),row.names = FALSE)
...
#The functions above should be run before executing the following codes
##Input the collected data
  plaza_s2<-c(12.5, 10.5, 11.7, 12.0, 10.4, 10.9)
  rainbow_s2<-c(11.4, 12.3, 10.1, 13.7, 13.2, 10.9, 11.3, 13.0)
  stateline_s2<-c(13.9, 14.0)
  
###First interim
  interim_2nd<-interim_analysis(interim = 2,
                                plaza=plaza_s2,
                                rainbow=rainbow_s2,
                                stateline=stateline_s2
                                )
...
####RAR
  Allocation_3<-allocation(interim_result=interim_2nd,
           new_ObsNum=8,
           block_size=4,
           fixed_allocation=1)
 flextable(Allocation_3)
 
```

### Third Interim Analysis  \
```{r Third Interim Analysis,eval = FALSE}
#The functions above should be run before executing the following codes
##Input the collected data
  plaza_s3<-c()
  rainbow_s3<-c()
...
###First interim
  interim_3rd<-interim_analysis(interim = 3,
                                plaza=plaza_s3,
                                rainbow=rainbow_s3,
                                stateline=stateline_s3
                                )

  flextable(interim_3rdt$Quantile)
  paged_table(interim_3rd$Sum_report)

####RAR
  Allocation_4<-allocation(interim_result=interim_3rd,
           new_ObsNum=8,
           block_size=4,
           fixed_allocation=1)
 flextable(Allocation_4)
 
```

### Final  Analysis  \
```{r Final Interim Analysis, eval = FALSE}
#The functions above should be run before executing the following codes
##Input the collected data
  plaza_s4<-c()
  rainbow_s4<-c()
...
###Final interim
  interim_4th<-interim_analysis(interim = 4,
                                plaza=plaza_s4,
                                rainbow=rainbow_s4,
                                stateline=stateline_s4
                                )

  flextable(interim_4th$Quantile)
  paged_table(interim_4th$Sum_report)
