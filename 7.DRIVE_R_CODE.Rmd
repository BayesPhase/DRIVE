...
This R code is for the DRIVE Trial Analysis. All the reports and simulations were run using R version 4.3.0 and FACTS (Berry Consultants, LLC, Austin, TX) version 7.0.0. The team replicated the FACTS simulations in R so that readers could replicate them. 
...
```
### Fixed Design \
The original trial used the adaptive design (see after fixed design code). However, anyone interested to do a fixed design, can follow the sample fixed design code below
```{r}
```
### Final Analysis Function for Fixed Design \
Function to perform the final analysis for fixed design
```{r Final Analysis function}
#The function above (Posterior function) should be run before executing the following codes
final_analysis_fixed <- function(plaza, rainbow, stateline){
  
  post <- post_dist(plaza, rainbow, stateline)
  
  plaza_post <- post$post_draw_theta$plaza
  rainbow_post <- post$post_draw_theta$rainbow
  stateline_post <- post$post_draw_theta$stateline
  sigma2_post <- post$post_draw_sigma2
  
  ##########################################################
  ### Posterior summaries
  ##########################################################
  df_summary <- data.frame(
    Parameter = c("theta0","theta1","theta2","sigma",
                  "theta1-theta0","theta2-theta0"),
    ...
    Estimate = c(mean(plaza_post),
             mean(rainbow_post),
             mean(stateline_post),
             sqrt(mean(sigma2_post)),
             mean(rainbow_post - plaza_post),
             mean(stateline_post - plaza_post)),
    
    quant0.025 = c(quantile(plaza_post,0.025),
            quantile(rainbow_post,0.025),
            quantile(stateline_post,0.025),
            sqrt(quantile(sigma2_post,0.025)),
            quantile(rainbow_post - plaza_post,0.025),
            quantile(stateline_post - plaza_post,0.025)),
    
    quant0.975 = c(quantile(plaza_post,0.975),
            quantile(rainbow_post,0.975),
            quantile(stateline_post,0.975),
            sqrt(quantile(sigma2_post,0.975)),
            quantile(rainbow_post - plaza_post,0.975),
            quantile(stateline_post - plaza_post,0.975))
  )
  
  ##########################################################
  ### Probability of being best
  ##########################################################
  prob_rainbow_best <- mean(rainbow_post < pmin(plaza_post, stateline_post))
  prob_stateline_best <- mean(stateline_post < pmin(plaza_post, rainbow_post))
  
  df_prob <- data.frame(
    Route = c("Rainbow","Stateline"),
    Pr_Best = c(prob_rainbow_best, prob_stateline_best)
  )
  
  ##########################################################
  ### final decision rule
  ##########################################################
  success_rainbow <- mean(rainbow_post < plaza_post) > 0.9812
  success_stateline <- mean(stateline_post < plaza_post) > 0.9812
  
  decision <- "Inconclusive / No clear winner"
  if(success_rainbow){
    decision <- "Rainbow better than control"
  } else if(success_stateline){
    decision <- "Stateline better than control"
  }
  
  df_decision <- data.frame(
    Decision = decision,
    Pr_rainbow_better = mean(rainbow_post < plaza_post),
    Pr_stateline_better = mean(stateline_post < plaza_post)
  )
...
### Final Analysis for Fixed Design \
```{r Final Analysis for fixed design}
#The functions above (Posterior function, One Final Analysis Function for Fixed Design ) should be run before executing the following codes
############################################################
### LOAD DATA
############################################################
library(rmarkdown)
...
Main_Data <- read_csv("S:/Biostats/BIO-STAT/BISR/FACTS Working Group/FACTS code/Case Study 22 DRIVE/2.Initial Designs/Fixed_Design_JZ/Fixed_Design_deterministic_results/Det Accrual 1_Drop1_Expected/patients00001.csv", skip = 1)

plaza <- Main_Data$`Visit 1`[Main_Data$Dose == 1]
rainbow <- Main_Data$`Visit 1`[Main_Data$Dose == 3]
stateline <- Main_Data$`Visit 1`[Main_Data$Dose == 2]

############################################################
### Final analysis at total sample size
############################################################
set.seed(1234)

final_result <- final_analysis_fixed(
  plaza = plaza,
  rainbow = rainbow,
  stateline = stateline
)
...
### OUTPUT
############################################################
paged_table(final_result$Summary)
paged_table(final_result$ProbBest)
paged_table(final_result$Decision)
...
#The functions above (Posterior function, Interim Analysis function, RAR Allocation function) should be run before executing the following codes
...
rainbow_s1<-Main_Data$time_diff[Main_Data$route_true_blinded==blinded_route[3]]
stateline_s1<-Main_Data$time_diff[Main_Data$route_true_blinded==blinded_route[2]]
...
plaza_s1<-Main_Data_new$time_diff[Main_Data_new$route_true_blinded==blinded_route[1]] 
rainbow_s1<-Main_Data_new$time_diff[Main_Data_new$route_true_blinded==blinded_route[3]]
stateline_s1<-Main_Data_new$time_diff[Main_Data_new$route_true_blinded==blinded_route[2]]
